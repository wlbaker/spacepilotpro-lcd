...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  for (int kk = 0; kk < 8; kk++) {
    ...
    if( kk%8 == 0 ) {
      for (int i=0;i<SPP_LCD_HEIGHT;i++) test_data[i][i] = 0xf0f0;
      renderText(test_data, txt = "HeLLo", 5, 50, 2, 0xfff0 );
    } else if( kk%8==1) {
      for (int i=0;i<SPP_LCD_HEIGHT;i++) test_data[i][SPP_LCD_HEIGHT - i - 1] = 0x0f00;
      renderText(test_data, txt = "hello", 5, 50, 2, 0xfff0 );
    } else if( kk%8==2) {
      for (int i=0;i<100;i++) test_data[80][ i] = 0xf000;
      renderText(test_data, txt = "world", 5, 50, 2, 0xfff0 );
    } else if( kk%8==3) {
      for (int i=0;i<100;i++) test_data[i][SPP_LCD_HEIGHT - i - 1] = 0xf000;
